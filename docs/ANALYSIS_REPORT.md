...
### 4.2 Easy 5장 vs Hard 15장 직접 비교

**Easy는 `simple` 5장, Hard는 `shadow`·`tilted`·`complex` 각 5장을 합친 15장**으로 정의한다. 성공 여부가 아니라 촬영 조건으로 나눈 구분이다. Easy에는 A4 문서 3장·영수증 1장·명함 1장, Hard에는 A4 문서 12장·영수증 3장이 포함된다. A4 문서는 Easy와 Hard 모두에 있어, A4 문서에서도 조명·원근 왜곡·배경 조건에 따른 차이를 살펴볼 수 있다.

| 구분 | 이미지 수 | 이미지 평균 밝기의 평균 | 이미지 표준편차의 평균 | 이미지 5백분위수의 평균 | 이미지 95백분위수의 평균 |
|---|---:|---:|---:|---:|---:|
| Easy | 5 | 123.77 | 87.15 | 25.80 | 229.20 |
| Hard | 15 | 132.87 | 75.55 | 34.27 | 228.67 |

각 값은 `eda.json`의 이미지별 통계를 그룹 내에서 산술평균했다. 배경을 포함한 통계이며, A4 문서 영역만의 밝기나 그룹 전체 픽셀의 통계는 아니다. Hard는 Easy보다 평균 밝기가 9.10 높고 이미지 표준편차의 평균은 11.60 낮다. 그러나 아래처럼 모든 설정에서 성공률은 더 낮아, 전체 이미지가 밝다고 A4 문서와 배경의 경계 검출이 쉬운 것은 아니다.

| 설정 | Easy 성공 / 전체 | Easy 성공률 | Hard 성공 / 전체 | Hard 성공률 | 성공률 차이 (Easy − Hard) |
|---|---:|---:|---:|---:|---:|
| default | 5 / 5 | 100% | 10 / 15 | 66.7% | 33.3%p |
| sensitive | 5 / 5 | 100% | 11 / 15 | 73.3% | 26.7%p |
| strict | 5 / 5 | 100% | 7 / 15 | 46.7% | 53.3%p |

Easy는 세 설정 모두 성공했지만 Hard에서는 sensitive가 가장 높은 성공률을 보였다. Hard의 A4 문서에는 그림자·반사, 구김·부분 가림, 배경의 사각형과 경쟁하는 사례가 포함되어 밝기 평균만으로 설명되지 않는 어려움이 있다. 표본 수가 5장과 15장으로 다르므로 성공 건수와 성공률을 함께 비교했다. 동일한 이미지에 세 설정을 적용했으며, 문서 종류와 촬영 조건이 섞인 소규모 관찰이므로 A4 문서 전체에 대한 일반화나 개별 조건의 인과 효과로 해석하지 않는다.
...
### 8.1 이미지는 `(H, W, C)` 형태의 NumPy 배열

OpenCV로 읽은 컬러 이미지는 NumPy의 3차원 배열이며 형태는 `(H, W, C)`이다. `H`는 세로 픽셀 수(height), `W`는 가로 픽셀 수(width), `C`는 채널 수(channel)를 뜻한다. 이 프로젝트의 입력은 `uint8` 자료형의 BGR 이미지이므로 `C=3`이고, 각 채널 값은 0~255 범위이다. 예를 들어 `image[y, x]`는 `(B, G, R)` 세 값을 가진 한 픽셀이고 `image.shape[:2]`는 `(H, W)`를 반환한다. `cvtColor(BGR2GRAY)`를 적용하면 채널 축이 사라져 grayscale 이미지는 `(H, W)` 형태의 2차원 배열이 된다.

### 8.2 Convolution은 국소 영역과 커널의 곱-합

Convolution은 작은 필터 커널을 이미지 위에서 이동시키며, 각 위치의 이미지 국소 영역과 커널에서 대응하는 원소끼리 곱한 뒤 모두 더하는 연산이다. 즉 펼쳐 놓은 두 배열의 내적과 같다. 입력 이미지가 $I$, 커널이 $K$라면 한 출력 픽셀은 개념적으로 $O(y,x)=\sum_i\sum_j I(y+i,x+j)K(i,j)$로 계산된다. 커널 값에 따라 주변 픽셀을 평균내어 잡음을 줄이거나, 밝기 변화가 큰 방향을 강조하여 경계를 찾을 수 있다.

이 파이프라인의 `GaussianBlur`는 가우시안 커널로 주변 픽셀의 가중합을 구해 영상을 부드럽게 만들고, Canny 내부의 미분 필터는 밝기 변화가 큰 경계를 찾는다. blur 커널이 커지면 더 넓은 국소 영역이 한 출력값에 영향을 주므로 작은 잡음은 줄지만 약한 문서 경계도 흐려질 수 있다. 이번 실험에서 blur 크기와 Canny 결과가 설정별로 달라진 이유가 이 국소 곱-합 연산과 연결된다.

### 8.3 Perspective transform은 좌표를 행렬로 바꾸는 기하 변환

Perspective transform은 이미지의 픽셀 값을 직접 고치는 것이 아니라, 원본 좌표가 출력 이미지의 어느 좌표로 이동할지를 3×3 행렬로 표현하는 기하 변환이다. 원본 점 $(x,y)$를 동차좌표 $(x,y,1)$로 나타내고 변환 행렬 $H$를 곱하면 $[u',v',w']^T=H[x,y,1]^T$가 되며, 최종 좌표는 $(u'/w',v'/w')$이다. 마지막 나눗셈이 있기 때문에 평행선의 간격이 촬영 방향에 따라 달라지는 원근 효과까지 표현할 수 있다.

이 프로젝트는 검출한 문서의 네 꼭짓점과 출력 직사각형의 네 점을 `getPerspectiveTransform`에 전달해 변환 행렬을 구하고, `warpPerspective`로 각 픽셀 좌표를 대응시켜 문서를 정면 형태로 보정한다. 따라서 행렬 계산 자체가 정확해도 입력 꼭짓점이 틀리면 잘못된 영역이 변환된다. `complex_3`의 받침 오인과 `shadow_1`의 부정확한 꼭짓점이 원근 보정 결과에도 그대로 이어진 이유이다.
